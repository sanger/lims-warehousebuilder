...
module Lims::WarehouseBuilder
  describe "Model::Gel" do
    ...
    it "returns a gel given its uuid" do
      object.save
      Model::Gel.gel_by_uuid(uuid).should be_a(Model::Gel)
      (Model::Gel.gel_by_uuid(uuid).values - [:internal_id]).should == (object.values - [:internal_id]) 
    end

    it "raises an error if the gel cannot be found" do
      expect do
        Model::Gel.gel_by_uuid(uuid)
